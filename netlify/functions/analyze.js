...
${buildSmartResultInstruction(mode, userLang)}
...
3) wypisz konkretne działania użytkownika (jasne kroki), ale tylko jeśli materiał rzeczywiście wymaga działania.
Dla zwykłego pisma możesz podać praktyczny blok „Co zrobić teraz?”.
Dla trybów umowy/PDF/formularza/rachunku nie dodawaj generycznych zaleceń typu logowanie do strony, banku lub telefonowanie, jeżeli materiał tego nie wskazuje.
...
detectedPhones,
    ...
    // Smart document tools always return one compact explanation followed by a faithful translation.
    // This is composed AFTER all language/safety post-processing so the translation cannot be lost
    // or replaced by the old six-card fallback in the frontend.
    payload.smartResult = composeSmartResultWithTranslation(
      mode, userLang, payload.smartResult, payload.translation, text, effectiveSource
    );
...
Rozpoznaj strony i rodzaj umowy. Wyciągnij przede wszystkim konkretne warunki: czas trwania, ceny/opłaty, terminy, wypowiedzenie, automatyczne przedłużenie, kary i inne zapisy mające realne znaczenie dla użytkownika.
Nie wymyślaj działań, kont internetowych, stron, procedur ani obowiązków, których nie ma w umowie. Jeśli czegoś brakuje, po prostu tego nie dodawaj.
...
Najpierw wewnętrznie rozpoznaj, czym rzeczywiście jest PDF, np. paskiem wynagrodzenia, pismem, umową, fakturą, formularzem, instrukcją, raportem, potwierdzeniem albo innym dokumentem.
Następnie wyciągnij tylko informacje istotne dla tego konkretnego rodzaju materiału. Nie przepisuj całego PDF-u i nie zwracaj surowego OCR-u.
Jeśli tekst zawiera oznaczenia stron, połącz informacje z całego dokumentu i nie pokazuj użytkownikowi markerów typu PAGE/STRONA.
Nie wymyślaj działań, stron internetowych ani procedur, których dokument nie podaje.`,

    document_form: `TRYB POMOCY PRZY FORMULARZU.
Wewnętrznie rozpoznaj cel formularza, widoczne pola, wymagane załączniki, podpisy i terminy.
Wyjaśnij praktycznie: co użytkownik ma wpisać, co dołączyć, gdzie podpisać i do kiedy złożyć formularz — ale tylko wtedy, gdy te informacje są rzeczywiście widoczne.
Nigdy nie wymyślaj danych osobowych użytkownika, stron internetowych, numerów kontaktowych ani procedur. Nie zgaduj nieczytelnych wartości i nie wypełniaj brakujących informacji za niego.`,
...
Wewnętrznie sprawdź rodzaj dokumentu, dostawcę, czego dotyczy opłata, sumę, wcześniejsze wpłaty lub korekty, realną kwotę pozostałą do zapłaty, termin, IBAN/referencję i ewentualne opłaty za spóźnienie.
Najważniejsze są: ile faktycznie trzeba zapłacić, do kiedy i jakie dane płatności trzeba wykorzystać. Nie każ użytkownikowi logować się do banku ani na stronę dostawcy, jeśli dokument tego nie wymaga.
...
function buildSmartResultInstruction(mode, userLang) {
  if (!["contract", "document_pdf", "document_form", "document_bill"].includes(mode)) return "";
  return `
DODATKOWE POLE smartResult:
- To ma być KRÓTKIE PRAKTYCZNE WYJAŚNIENIE dla użytkownika w języku ${userLang}. Nie umieszczaj w smartResult pełnego tłumaczenia dokumentu — backend dołączy je osobno.
- Wybierz tylko 2-5 najważniejszych sekcji pasujących do konkretnego materiału; pomijaj sekcje puste lub nieistotne.
- Używaj prostych krótkich nagłówków i punktów •, bez składni Markdown typu ** lub ###.
- Nie powtarzaj całego dokumentu, nie pokazuj OCR-u, markerów stron ani wewnętrznej checklisty.
- Zachowaj dokładnie ważne kwoty, daty, terminy, nazwy i warunki.
- Nie wymyślaj kroków, stron, kont, numerów telefonu, urzędów, sposobu wysyłki, obowiązków ani ryzyka, którego materiał nie uzasadnia.
- Nie dodawaj ogólników typu „upewnij się, że dane są poprawne”, jeśli nie pomagają zrozumieć konkretnego materiału.
- Gdy coś jest niepewne, napisz to krótko zamiast zgadywać.
- Dla umowy priorytet: najważniejsze warunki, opłaty, wypowiedzenie/odnowienie, kary i konkretne terminy.
- Dla PDF priorytet zależy od rozpoznanego rodzaju dokumentu; pokaż esencję, nie transkrypcję.
- Dla formularza wypisz konkretnie: co wpisać lub zaznaczyć, jakie załączniki są wymagane, gdzie jest podpis, jakie są opłaty i terminy. Jeśli formularz NIE podaje sposobu lub miejsca złożenia, nie wymyślaj go.
- Dla rachunku/faktury priorytet: kwota faktycznie do zapłaty, termin, wcześniejsze wpłaty/korekty, IBAN/referencja oraz konkretne opłaty za spóźnienie.
`;
}

function smartTranslationHeading(userLang) {
  const L = String(userLang || "EN").toUpperCase();
  const labels = {
    PL:"Tłumaczenie dokumentu", UA:"Переклад документа", NL:"Vertaling van het document", EN:"Document translation",
    DE:"Übersetzung des Dokuments", FR:"Traduction du document", IT:"Traduzione del documento", ES:"Traducción del documento",
    PT:"Tradução do documento", LT:"Dokumento vertimas", LV:"Dokumenta tulkojums", HU:"A dokumentum fordítása",
    ZH:"文件翻译", JA:"文書の翻訳", HI:"दस्तावेज़ का अनुवाद", AR:"ترجمة المستند", EG:"ترجمة المستند",
    ET:"Dokumendi tõlge", RO:"Traducerea documentului", HR:"Prijevod dokumenta", FI:"Asiakirjan käännös",
    SV:"Översättning av dokumentet", NO:"Oversettelse av dokumentet", DA:"Oversættelse af dokumentet"
  };
  return labels[L] || labels.EN;
}

function composeSmartResultWithTranslation(mode, userLang, smartResult, translation, sourceText, effectiveSource) {
  if (!["contract", "document_pdf", "document_form", "document_bill"].includes(mode)) return cleanAiText(str(smartResult || ""));

  const analysis = cleanAiText(str(smartResult || ""));
  const translated = cleanAiText(str(translation || ""));
  const original = cleanAiText(str(sourceText || ""));
  const source = String(effectiveSource || "UNKNOWN").toUpperCase();
  const target = String(userLang || "AUTO").toUpperCase();

  // If source and target are the same, duplicating the whole document adds no value.
  const needsTranslation = translated && target !== "AUTO" && source !== target && translated !== original;
  if (!needsTranslation) return analysis;

  const heading = `🌍 ${smartTranslationHeading(target)}`;
  return [analysis, `${heading}\n${translated}`].filter(Boolean).join("\n\n");
}
...
async function sbFinalUserLanguageGuardV4(apiKey, payload, userLang) {
  ...
  const source = {
    ...
    smartResult: payload.smartResult || "",
...
`.trim();
  ...
  try {
    ...
    if (cleanStr(translated.smartResult)) payload.smartResult = cleanStr(translated.smartResult);
